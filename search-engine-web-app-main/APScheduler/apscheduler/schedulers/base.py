...
from APScheduler.apscheduler.schedulers import SchedulerAlreadyRunningError, SchedulerNotRunningError
from APScheduler.apscheduler.executors.base import MaxInstancesReachedError, BaseExecutor
from APScheduler.apscheduler.executors.pool import ThreadPoolExecutor
from APScheduler.apscheduler.jobstores.base import ConflictingIdError, JobLookupError, BaseJobStore
from APScheduler.apscheduler.jobstores.memory import MemoryJobStore
from APScheduler.apscheduler.job import Job
from APScheduler.apscheduler.triggers.base import BaseTrigger
from APScheduler.apscheduler.util import (
    asbool, asint, astimezone, maybe_ref, timedelta_seconds, undefined, TIMEOUT_MAX)
from APScheduler.apscheduler.events import (
...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    if (3, 8) <= sys.version_info < (3, 10):
        _trigger_plugins = {ep.name: ep for ep in entry_points()['APScheduler.apscheduler.triggers']}
        _executor_plugins = {ep.name: ep for ep in entry_points()['APScheduler.apscheduler.executors']}
        _jobstore_plugins = {ep.name: ep for ep in entry_points()['APScheduler.apscheduler.jobstores']}
    else:
        _trigger_plugins = {ep.name: ep for ep in entry_points(group='APScheduler.apscheduler.triggers')}
        _executor_plugins = {ep.name: ep for ep in entry_points(group='APScheduler.apscheduler.executors')}
        _jobstore_plugins = {ep.name: ep for ep in entry_points(group='APScheduler.apscheduler.jobstores')}
