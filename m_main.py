...
def main():
    ...
    for query in our_query_df.our_query_id.unique():
        ...
        # dataframe with all the documents considered by the rank docs algorithm of the query QX
        ...
        # True Positive: relevant documents classified as relevant
        tp = query_x_df[(query_x_df['predicted'] == 1) & (query_x_df['our_query_id'] == query)]['label'].sum()

        # False Negative: relevant documents classified as non-relevant 
        fn =  query_x_df[(query_x_df['predicted'] == 0) & (query_x_df['our_query_id'] == query)]['label'].sum()

        # False Positive: non-relevant documents classified as relevant
        fp = query_x_df[(query_x_df['predicted'] == 1) & (query_x_df['label'] == 0)]['predicted'].sum()
        ...
        labels = np.array(sorted_adapted_df[sorted_adapted_df['our_query_id'] == query]["label"])
        scores = np.array(sorted_adapted_df[sorted_adapted_df['our_query_id'] == query]["predicted"])
        ndcg_k = np.round(ndcg_at_k(ground_truth, scores, top), 4)
        print("ndcg@{} for query with query_id={}: {}".format(top, query, ndcg_k))
